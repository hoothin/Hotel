...
class WxpayService{
    ...
    private $type;//1 snsapi_base 2 snsapi_userinfo
    ...
    static public function initService(){
        if (!self::$service instanceof self) {
            global $mchid;
            global $appid;
            global $appKey;
            global $apiKey;
            self::$service = new self($mchid, $appid, $appKey,$apiKey);
        }
        return self::$service;
    }
     ...
     * @return 用户的openid
     */
    public function GetOpenid($type=1){
        $this->type=$type;
        //通过code获得openid
        if (!isset($_GET['code'])){
            //触发微信返回code码
    ...
    public function getUserinfo(){
        $this->GetOpenid(2);
        if(!$this->data || !$this->data["openid"]){
            $scheme = $_SERVER['HTTPS']=='on' ? 'https://' : 'http://';
            $baseUrl = $scheme.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?".explode("code",$_SERVER['QUERY_STRING'])[0];
    ...
    private function __CreateOauthUrlForCode($redirectUrl){
        ...
        $urlObj["scope"] = $this->type==1?"snsapi_base":"snsapi_userinfo";
    ...
    public function notify(){
        ...
        if(!$postStr)$postStr=file_get_contents("php://input");
