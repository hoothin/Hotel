        ...
        var calendarSwitch = (function() {
                ...
                calendarSwitch.prototype = {
                    init: function() {
                        ...
                        var html = "<div class='headerWrapper'><div class='headerTip'>入住离店日期</div><div class='comfire'>完成</div></div><table class='dateZone'><tr><td class='colo'>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td class='colo'>六</td></tr></table>" + "<div class='tbody'></div>"
                        ...
                        for (var q = 0; q < me.index; q++) {
                            ...
                            for (var m = 0; m < DaysInMonth[currentMonth]; m++) {
                                var curDay=arryTd.eq(firstDay++);
                                curDay.text(m + 1);
                                var lunarRes=LunarCalendar.solarToLunar(currentYear,yf,m+1);
                                curDay.append("<p class='holiday'>"+(lunarRes.solarFestival?lunarRes.solarFestival:"")+(lunarRes.lunarFestival?lunarRes.lunarFestival:"")+"</p>");
                            }
                        }
                        me._initselected();
                        ...
                        $(me.comfire).on('click', function(event) {
                            ...
                            if ($(me.sections).find('.tbody .rz').length < 2) {
                                $('.aui-calendar-tips').fadeIn(600);
                                setTimeout(function() {
                                    $('.aui-calendar-tips').fadeOut(600);
                                }, 1500)
                                return false;
                            }
                            $(me.sections).find('.tbody .rz').each(function(index, element) {
                                if ($(this).text() == '离店') {
                                    $(me.sections).find('.tbody .rz').each(function(index, element) {
                                        ...
                                        if ($(this).text() == '离店') {
                                            var day = $(this).parent().prop('firstChild').nodeValue;
                                            if (day < 10) {
                                                day = "0" + day;
                                            }
                    ...
                    _initselected: function() {
                        ...
                        me.maxDays = me.settings.maxDays;
                        me.fullState = me.settings.fullState;
                        ...
                        if (me.daysnumber) {
                            ...
                            for (var i = strDays - 1; i < strDays + parseInt(me.daysnumber); i++) {
                                arry1.push(arry[i]);
                                if(me.fullState && !me.fullState[j]){
                                    $(arry[i]).append("<p class='full'>满房</p>");
                                }
                                j++;
                            }
                            for (var i = strDays + parseInt(me.daysnumber)-1; i < $(arry).length; i++) {
                                $(arry[i]).css('color', '#ccc');
                            }
                        } else {
                    ...
                    _selectDate: function(arry1) {
                        ...
                        $(arry1).on('click', function(index) {
                            ...
                            if(me.maxDays==1 && first+1==arry1.length)return;
                            if ($('.calendar').css('display') != "none") {
                                if (flag == 0) {
                                    ...
                                    me._checkColor(me.comeColor, me.outColor)
                                    if(me.maxDays==1){
                                        var end=arry1[first+1];
                                        if(first+1 != arry1.length-1){
                                            $(arry1[arry1.length-1]).css({
                                                'background': '#fff',
                                                'color': '#ccc'
                                            });
                                        }
                                        $(end).append('<p class="rz">离店</p>');
                                        $(me.sections).find('.rz').each(function(index, element) {
                                            if ($(end).text() == '离店') {
                                                $(end).parent('td').append('<span class="hover lidian_hover">共1晚</span>');
                                                $(end).parent('td').css('position', 'relative');
                                            }
                                        });
                                        $('.hover').css({
                                            'position': 'absolute',
